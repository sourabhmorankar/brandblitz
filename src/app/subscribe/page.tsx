...
const subscriptionPlans = [
  { name: 'Weekly', price: 999, requests: 1, planId: 'P-123456789' },    // Replace with actual Plan ID
  { name: 'Weekly+', price: 1899, requests: 2, planId: 'P-987654321' },  // Replace with actual Plan ID
  { name: 'Weekly++', price: 2699, requests: 3, planId: 'P-456789123' }, // Replace with actual Plan ID
];
...
const SubscribePage = () => {
  ...
  const createSubscription: PayPalButtonsComponentProps['createSubscription'] = (_data, actions) => {
    return actions.subscription.create({
      ...
      custom_id: user.uid, // Pass user ID for webhook
    });
  };

  const onApprove: PayPalButtonsComponentProps['onApprove'] = async (data) => {
    console.log('Subscription approved:', data);
    // Webhook will handle Firestore update
  };
          ...
          {subscriptionPlans.map((plan) => (
            <div
              ...
              className={`card cursor-pointer transition-all duration-200 ${
                selectedPlan.name === plan.name ? 'border-indigo-400' : ''
              }`}
              onClick={() => setSelectedPlan(plan)}
            >
              <h2 className="text-xl font-semibold text-gray-200 mb-2">{plan.name}</h2>
